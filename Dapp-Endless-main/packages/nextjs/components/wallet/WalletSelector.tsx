...
// 重命名为自定义的 formatAddress 函数
const formatAddress = (address?: string | any): string => {
  if (!address) return "Connect Wallet";
  
  // 如果address不是字符串，尝试转换或返回默认值
  if (typeof address !== 'string') {
    // 如果是对象且有toString方法
    if (address && typeof address === 'object' && address.toString) {
  ...
  return connected ? (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button>
          {account?.ansName || formatAddress(account?.address) || "Connect Wallet"}
        </Button>
      </DropdownMenuTrigger>
